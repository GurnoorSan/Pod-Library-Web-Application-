...
class Episode:
    def __init__(self, episode_id: int, podcast_id: int, episode_title: str = "Untitled", episode_audio: str = "No link",
    ...
    def __repr__(self):
        return f"<Episode {self._id}: '{self._title}' from podcast {self._podcast_id}>"
    ...
    def __hash__(self):
        return hash((self.id, self.podcast_id, self.upload_date))


class Review:
    def __init__(self, review_id: int, user: User, rating: float, content: str,
...
def make_review(review_id: int, content: str, user: User, podcast: Podcast, episode: Episode, rating: float):
    review = Review(review_id, user, rating, content, podcast, episode)
