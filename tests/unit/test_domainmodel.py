...
def test_episode_initialisation(my_episode, my_podcast):
    ...
    assert my_episode.podcast_id == 100
    assert repr(my_episode) == "<Episode 1: 'Untitled' from podcast 100>"

    episode2 = Episode(2, my_podcast.id, "To be or not to be ")
    with pytest.raises(ValueError):
        episode3 = Episode(-2, my_podcast.id)

    episode4 = Episode(4, my_podcast.id, "That is the question", "https://audiojungle.com",
...
def test_episode_equality(my_episode, my_podcast):
    episode1 = Episode(2, my_podcast.id)
    assert episode1 != my_episode
    episode2 = Episode(1, 101)  # In practice this cannot exist; only one episode of each id.
    assert episode2 == my_episode
    episode3 = Episode(1, 100)
    assert episode3 == my_episode
    assert my_episode != "Banana"
    assert my_episode != repr(my_episode)


def test_episode_inequality(my_episode, my_podcast):
    episode1 = Episode(2, my_podcast.id)
    assert episode1 > my_episode
    episode2 = Episode(1, 99)
    assert episode2 < episode1
    assert not episode2 < "Banana"
    assert not episode1 > repr(my_episode)


def test_episode_hash(my_episode, my_podcast):
    episode1 = Episode(2, my_podcast.id)
    assert hash(episode1) != hash(my_episode)
    episode2 = Episode(1, my_podcast.id)
    assert hash(episode2) == hash(my_episode)
    episode3 = Episode(1, 99)
...
def test_playlist_get_total_runtime(my_playlist, my_episode):
    assert my_playlist.get_total_runtime() == 0
    my_playlist.add_episode(my_episode)
    assert my_playlist.get_total_runtime() == 0
    episode1 = Episode(2, 100, "Episode 2", "No link", 100)
    episode2 = Episode(3, 100, "Episode 3", "No link", 200)
    my_playlist.add_episode(episode1)
...
def test_playlist_swap_element(my_playlist, my_podcast):
    episode1 = Episode(2, my_podcast.id, "Apple")
    episode2 = Episode(3, my_podcast.id, "Banana")
    episode3 = Episode(4, my_podcast.id, "Orange")
    my_playlist.add_episode(episode1)
    my_playlist.add_episode(episode2)
