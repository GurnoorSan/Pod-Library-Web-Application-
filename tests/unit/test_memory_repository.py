...
from podcast.adapters import datareader
...
@pytest.fixture
def episode():
    episode =Episode(1, 1, "D-Hour Radio Network")
    return episode

@pytest.fixture
def episode_2():
    episode =Episode(2, 1, "Brian Denny Radio")
    return episode
...
def test_get_number_of_podcasts(in_memory_repo, author, author_2, podcast_2):
    assert in_memory_repo.get_number_of_podcasts() == 6
    ...
    assert in_memory_repo.get_number_of_podcasts() == test_num + 7
...
def test_add_and_get_episode(in_memory_repo, podcast, podcast_2, episode, episode_2):
    assert in_memory_repo.get_episode(1) is None  #checks if episode is none
    in_memory_repo.add_podcast(podcast)
    in_memory_repo.add_episode(episode)  #add podcast and episode
    get_episode = in_memory_repo.get_episode(1) #get episode
    assert get_episode == episode #check to see if get_episode is = episode
    assert in_memory_repo.get_episode(2) is None #check to see no other episodes were added
    in_memory_repo.add_podcast(podcast_2)
    in_memory_repo.add_episode(episode_2)
    get_episode_2 = in_memory_repo.get_episode(2)
    assert get_episode_2 != episode
    assert get_episode != episode_2   #cross checks to see if any errors occur when making multiples episodes


def test_get_number_of_episodes(in_memory_repo, podcast, episode, episode_2):
    assert in_memory_repo.get_number_of_episodes() == 0
    in_memory_repo.add_podcast(podcast)
    in_memory_repo.add_episode(episode)
    assert in_memory_repo.get_number_of_episodes() == 1
    in_memory_repo.add_episode(episode_2)
    assert in_memory_repo.get_number_of_episodes() == 2   #makes episodes and adds them to memory repo then gets the number of episodes
...
def test_populate(in_memory_repo):
    ...
    in_memory_repo.populate(podcasts_file)
